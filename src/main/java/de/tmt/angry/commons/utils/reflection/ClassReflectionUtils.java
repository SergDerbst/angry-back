...
public class ClassReflectionUtils {
	...
	private static List<Annotation> retrieveActiveParentAnnotations(final Class<?> parentClass, final List<Annotation> list) {
		for (final Annotation annotation : parentClass.getAnnotations()) {
			if (annotation.getClass().isAnnotationPresent(Inherited.class)) {
				list.add(annotation);
			}
		}
		if (!parentClass.getSuperclass().equals(Object.class)) {
